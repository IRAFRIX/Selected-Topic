  ...
  const onSubmitForm = (data) => {
    ...
    for (let f of inputFile.current.files) {
      if (f.size > maxSize * 1000) {
        alert(`ไฟล์ขนาดใหญ่เกิน ${maxSize} KB`);
        return;
      }
    }
    console.log(data);
    ...
    <div className="mt-3 mb-5 mx-auto p-3 rounded" style={{ width: '400px', background: '#cee' }}>
      <form onSubmit={handleSubmit(onSubmitForm)}>
        <div className="form-group mb-3">
          ...
          {errors.category && <div style={err}>กรุณาเลือกหมวดหมู่</div>}
        </div>
        <div className="form-group mb-3">
          ...
          {opts.map((opt, i) => {
            return (
              <div className="form-check form-check-inline mb-2" key={i}>
                <input type="checkbox" id={'opt' + i} value={opt} className="form-check-input" {...register(`option${i}`)} />
